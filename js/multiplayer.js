...
$(window).keydown(function (ev) {
    ...
    if (ev.which == 13) { // Enter key pressed
        ...
        ev.preventDefault();
    } else if (ev.which == 27) {//Escape key pressed
...
var multiplayer = {
    ...
    //websocket_url: "ws://192.168.1.101:4000",
//    websocket_url: "ws://172.16.111.201:4000",
...
//            multiplayer.endGame("Error connecting to server.");
            multiplayer.endGame("连接到服务器发生错误。");
        }
...
//            multiplayer.endGame("Error connecting to server");
    ...
    startGame: function () {
        fog.initLevel();
        game.animationLoop();
        multiplayer.animationInterval
            = setInterval(multiplayer.tickLoop, game.animationTimeout);
        game.start();
    },
...
//    },
    ...
    updateRoomStatus: function (status) {
        ...
        for (var i = 0; i < status.length; i++) {
            var key = "房间 " + (i + 1) + ". " + this.statusMessages[status[i]];
...
//            game.showMessageBox("Please select a game room to join.");
    ...
    closeAndExit: function () {
        ...
        document.getElementById('multiplayergameslist').disable = false;
        document.getElementById('multiplayerjoin').disable = false;
