...
var fog = {
    ...
    isPointOverFog: function (x, y) {
        if (!this.isSpySatellite) {
            // If the point is outside the map bounds consider it fogged
            if (y < 0 || y / game.gridSize >= game.currentLevel.mapGridHeight
                || x < 0 || x / game.gridSize >= game.currentLevel.mapGridWidth) {
                return true;
            }
    ...
    animate: function () {
        ...
        for (var i = 0; i <= game.items.length - 1; i++) {
            ...
            if (item.name == "ground-radar" && item.team == game.team && item.lifeCode == "healthy") {
                radar = true;
                break;
            }
        }
        if (/*radar*/true) {
            fog.isSpySatellite = true;
        } else {
            fog.isSpySatellite = false;
        }
