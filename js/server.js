...
// Create a simple web server that return the same response for any request
...
wsServer.on('request', function (request) {
            ...
            switch (clientMessage.type) {
                ...
                case "command":
                    if (player.room && player.room.status == "running") {
                        ...
                        player.room.lastTickConfirmed[player.color]
                            = clientMessage.currentTick + player.tickLag;
                    }
                    break;
                case "lose_game":
                ...
                case "chat":
                    if (player.room && player.room.status == "running") {
                        ...
                        sendRoomWebSocketMessage(player.room,
                            {type: "chat", from: player.color, message: cleanedMessage});
                    }
                    break;
            }
        }
    ...
    connection.on('close', function (reasonCode, description) {
        ...
        // If the player is in room, remove him from room and notify everyone
        if (player.room) {
...
function joinRoom(player, roomId) {
    ...
    console.log("添加玩家 "+player.connection.remoteAddress+" 到房间： ", roomId);
...
function initGame(room) {
    ...
    console.log("两个玩家已经加入. 从房间 " + room.roomId+" 开始初始化游戏");
    ...
    // Randomly select who spawn locations between 0 and 3 for both players
    var spawns = [0, 1, 2, 3];
    var spawnLocations = {
    ...
    sendRoomWebSocketMessage(room,
        {type: "init_level", spawnLocations: spawnLocations, level: currentLevel});
}
...
function finishMeasuringLatency(player, clientMessage) {
    ...
    console.log("Measuring Latency for player. Attempt",
        player.latencyTrips.length,
        "-Average Latency:",
        player.averageLatency,
        "Tick Lag:", player.tickLag);
}
