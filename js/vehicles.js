...
var vehicles = {
    ...
    defaults: {
        ...
        drawSelection: function () {
            ...
            game.foregroundContext.beginPath();
            game.foregroundContext.arc(x, y, this.radius + 1, 0, Math.PI * 2, false);
        ...
        processOrders: function () {
            ...
            switch (this.orders.type) {
                case "move":
                    ...
                    if (distanceFromDestinationSquared < Math.pow(this.radius / game.gridSize, 2)) {
                        ...
                        if (this.colliding && (Math.pow(this.orders.to.x - this.x, 2)
                            ...
                            // Count collision within 5 radius distance of goal
                    ...
                    if ((Math.pow(this.orders.to.x - this.x, 2) + Math.pow(this.orders.to.y - this.y, 2))
                        ...
                        if (Math.abs(difference) > turnAmount) {
                            ...
                            if (!this.reloadTimeLeft) {
                                ...
                                var angleRadius = -(Math.round(this.direction) / this.direction) * 2 * Math.PI
                ...
                case "patrol":
                    ...
                    if ((Math.pow(this.orders.to.x - this.x, 2) + Math.pow(this.orders.y - this.y, 2))
                        < Math.pow(this.radius * 4 / game.gridSize, 2)) {
        ...
        moveTo: function (destination) {
            ...
            var newDirection;
            // if vehicles is outside map bounds, just go straight towards goal
            if (start[1] < 0
                || start[1] >= game.currentLevel.mapGridHeight
                || start[0] < 0
                ...
                if (this.orders.path.length > 1) {
                    ...
                    // Reached destination grid;   //到达目的地
                    this.orders.path = [this, destination];
                    newDirection = findAngle(destination, this, this.directions);
                } else {
                    // There is not path  //没有找到路径
                    return false;
                }
            }
            ...
            if (collisionObjects.length > 0) {
                ...
                collisionObjects.push({collisionType: "attraction",
            ...
            if (this.hardCollision) {
                // in case of hard collision, do not move toward, just turn towards new direction
                if (Math.abs(difference) > turnAmount) {
                    this.direction = wrapDirection(this.direction + turnAmount * Math.abs(difference) / difference, this.directions);
                }
            } else {
        ...
        /**
         *  Find out collision objects
         *  检查将要碰撞的 对象
         */
        checkCollisionObjects: function (grid) {
            // Make a list of collisions that the vehicles will have if it goes along present path
            var movement = this.speed * game.speedAdjustmentFactor;
            var angleRadius = -(Math.round(this.direction) / this.directions) * 2 * Math.PI;
            var newX = this.x - (movement * Math.sin(angleRadius));
            var newY = this.y - (movement * Math.cos(angleRadius));
            ...
            // Test grid up to 3 squares away
            for (var j = x1; j < x2; j++) {
                for (var i = y1; i <= y2; i++) {
                    if (grid[i][j] == 1) { // grid square is obstructed
                        if (Math.pow(j + 0.5 - newX, 2) + Math.pow(i + 0.5 - newY, 2)
                            ...
                            // Distance of obstructed grid form vehicles is less than hard collision threshold
            ...
            for (var i = game.vehicles.length - 1; i >= 0; i--) {
                ...
                // Test vehicles that are less than 3 squares away of collisions
                if (vehicle != this && Math.abs(vehicle.x - this.x) < 3 && Math.abs(vehicle.y - this.y) < 3) {
                    if (Math.pow(vehicle.x - newX, 2) + Math.pow(vehicle.y - newY, 2)
                        ...
                        // Distance between vehicles is less than hard collision threshold (sum of vehicle ardii)
